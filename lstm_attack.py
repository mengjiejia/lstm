...
# import dataset
df_train = pd.read_csv('log_8_2022-3-12-17-48-52_normal_data_last.csv')
...
df_train.rename(columns={'Unnamed: 0': 'datasource'}, inplace=True)
...
#_test_input, y_test_input = process_data(X_test, y_test, stride)
...
for i in range(0, len(X_train_input)):
    ...
    loss = criterion(output.view(-1), y_batch)
    err = output.view(-1)-y_batch
    err1 = torch.sign(loss)
    #print("Error", err)
    #print(output.view(-1))
    if i == 0:
...
for b in range(0, len(X_valid_input)):
    ...
    x_valid = x_valid + (0.3 * err1)
    #print("ValidX", x_valid)
