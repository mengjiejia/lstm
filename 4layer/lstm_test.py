...
class Sequence(torch.nn.Module):
    def __init__(self):
        ...
        self.lstm1 = torch.nn.LSTMCell(11, 256)
        self.drop_out1 = torch.nn.Dropout(0.2)
        self.lstm2 = torch.nn.LSTMCell(256, 128)
        self.drop_out2 = torch.nn.Dropout(0.2)
        self.lstm3 = torch.nn.LSTMCell(128, 64)
        self.drop_out3 = torch.nn.Dropout(0.2)
        self.lstm4 = torch.nn.LSTMCell(64, 32)
    ...
    def forward(self, input):
        # Initial cell states
        h_t1 = torch.zeros(input.size(1), 256, dtype=torch.float32).to(self.device)
        c_t1 = torch.zeros(input.size(1), 256, dtype=torch.float32).to(self.device)
        h_t2 = torch.zeros(input.size(1), 128, dtype=torch.float32).to(self.device)
        c_t2 = torch.zeros(input.size(1), 128, dtype=torch.float32).to(self.device)
        h_t3 = torch.zeros(input.size(1), 64, dtype=torch.float32).to(self.device)
        c_t3 = torch.zeros(input.size(1), 64, dtype=torch.float32).to(self.device)
        h_t4 = torch.zeros(input.size(1), 32, dtype=torch.float32).to(self.device)
        c_t4 = torch.zeros(input.size(1), 32, dtype=torch.float32).to(self.device)
        outputs = []
        ...
        for i in range(input.size(0)):
            ...
            h_t3 = self.drop_out3(h_t3)
            h_t4, c_t4 = self.lstm4(h_t3, (h_t4, c_t4))
            output = self.fc1(h_t4)
...
for i in range(2000, 4000):
    for j in range(stride):  # 3 is the roll_rate index
        #for k in range(11):
        X_test_input[i][j][0] = X_test_input[i][j][0]* 3;
    y_test_input[i] = y_test_input[i] * 3
...
batch_size = 16
#PATH = 'TEST.pth'
PATH = 'test_samenorandom_cut_fixed_wing_custom_loss_sequence_training4layer_model.pth'
test_path = PATH
