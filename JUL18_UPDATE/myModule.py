...
class model2(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        # last sample as input
        h_t1, c_t1 = self.lstm1(input[seq_length - 1], (h_t1, c_t1))
        h_t2, c_t2 = self.lstm2(h_t1, (h_t2, c_t2))
        output = self.fc1(h_t2)
        output = self.fc2(output)
...
# first 5 dpts prediction
class turn_model2(torch.nn.Module):
    def __init__(self):
        super(turn_model2, self).__init__()
        self.lstm1 = torch.nn.LSTMCell(10, 8)
        self.lstm2 = torch.nn.LSTMCell(8, 8)
        self.fc1 = torch.nn.Linear(8, 4)
        self.fc2 = torch.nn.Linear(4, 1)
...
def regression_reconstruct_data(data, y_training, stride):
    X = []
    y = []
    L = len(data)
    for i in range(L - stride):
...
def mark_two_line(title, xlabel, ylabel, prediction, abnormal, start, end, filename, path, abnormal_mark, TP_list,
    ...
    prediction = [None] * stride + prediction
...
def two_line(title, xlabel, ylabel, prediction, normal, noattack, filename, path, stride, FP_list):
    ...
    plt.plot(np.arange(len(noattack)), noattack, 'g', linewidth=2.0, label='normal_sensor_reading')
    pair = []
    # for i in abnormal_mark:
    #     pair.append((prediction[i], abnormal[i]))
