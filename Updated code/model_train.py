...
directory = 'sequence/range_scale/normal_training/turn3_roll'
...
df_train1 = pd.read_csv(read_path + filename)  # turn cut 800 - 2450

X_training, nav_cmd_train = myModule.preprocess(df_train1)

# cut off the landing part
X_training = X_training[800:2450]
X_training_normal = X_training.copy()
y_train = X_training_normal[:, y_index]
print(np.array(y_train).shape)

# turning stage index
turn = [[125, 187], [257, 319], [408, 475], [516, 578], [626, 690], [738, 801], [872, 935], [962, 1026], [1100, 1163],
        [1200, 1265], [1322, 1386], [1424, 1489], [1565, 1628], [1655, 1684]]

# stable stage index
stable = [[188, 256], [320, 407], [476, 515], [579, 625], [691, 737], [802, 871], [935, 961], [1027, 1099],
          [1164, 1199], [1266, 1321], [1387, 1423], [1490, 1564], [1629, 1654]]
...
for pair in turn:
    x = X_training[pair[0]:pair[1]]
    y = y_train[pair[0]:pair[1]]
    turn_x.append(x)
    turn_y.append(y)
...
X_training_abnormal, X_valid_abnormal, y_training_normal, y_valid_normal = train_test_split(X_training_abnormal,
                                                                                                y_training,
                                                                                                test_size=0.2,
                                                                                                shuffle=False,
                                                                                                random_state=0)
