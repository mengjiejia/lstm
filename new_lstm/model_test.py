...
for i in range(2000, 4000):
    for j in range(stride):  # 3 is the roll_rate index
        # for k in range(11):
        X_test[i][j][0] = X_test[i][j][0] * 2
    y_test_abnormal[i] = y_test_abnormal[i] * 2
...
myModule.test_two_line('test result', x_label, y_label, test_y_prediction, y_test_abnormal, 2000, 4000, 'test result',
                       output_dir)
myModule.two_line('test result with groundtruth', x_label, y_label, test_y_prediction, y_test, 'test result with groundtruth', output_dir)
